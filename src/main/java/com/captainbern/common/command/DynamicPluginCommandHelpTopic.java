...
public class DynamicPluginCommandHelpTopic extends HelpTopic {
    ...
    public DynamicPluginCommandHelpTopic(DynamicPluginCommand cmd) {
        ...
        if (cmd.getRegisteredWith() instanceof CommandManager) {
            Map<String, String> helpText = ((CommandManager) cmd.getRegisteredWith()).getCommandRegistrationService().getHelpMessages();
            ...
            helpText = ((CommandManager) cmd.getRegisteredWith()).getCommandRegistrationService().getCommands();
            ...
            if (cmd.getRegisteredWith() instanceof CommandManager) {
                try {
                    for (String perm : cmd.getPermissions()) {
                        if (((CommandManager) cmd.getRegisteredWith()).hasPermission(perm, player)) {
                            return true;
                        }
                    }
                } catch (Throwable t) {
